...
public class ExportExcelMax {  
    ...
    //构造方法，传入要导出的数据  
    public ExportExcelMax( XSSFWorkbook workbook ,String title,String[] rowName,List<Object[]>  dataList, HttpServletResponse response){
    ...
    public void export( int a ) throws Exception{ 
        try{  
          ...
          // 创建工作簿对象  03版excel
            XSSFSheet sheet = workbook.createSheet("当前为第"+a+"页");                 // 创建工作表
...
//                    }  
	...
	private    void responseDown(XSSFWorkbook workbook) {
		if(workbook !=null){  
		    try  
		    {  
